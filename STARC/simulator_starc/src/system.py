...
class System:
    ...
    def set_accelerator(self, modelinfos, name: DeviceType, config):
        ...
        if self.hetero_name == DeviceType.PIM:
            ...
            # Optional knobs: populated by main.py after System construction.
            ramulator.sparsity = getattr(self, "sparsity", True)
            ramulator.kv_budget = getattr(self, "kv_budget", 1024)
            ramulator.row_size = getattr(self, "row_size", 16)
            ramulator.sparsity_ratio = getattr(self, "sparsity_ratio", 0.25)
            ramulator.kv_budget_dict = getattr(self, "kv_budget_dict", {})
            ramulator.cluster_mode = getattr(self, "cluster_mode", "none")

            self.devices["Acc"] = PIM(config, self.scaling_factor, ramulator)

        elif self.hetero_name == DeviceType.CPU:
            self.devices["Acc"] = xPU(DeviceType.CPU, config, self.scaling_factor)

    def set_xpu(self, config):
        # Force homogeneous GPU execution.
    ...
    def simulate(
        ...
        def add_infos(name, infos, time, energy, bound):
            ...
            if new_name in infos.keys():
                ...
                infos[new_name]["energy"] = [
                    eng + energy[i] for i, eng in enumerate(infos[new_name]["energy"])
                ]
            else:
                infos[new_name] = {"time": time, "energy": energy, "bound": bound}
        ...
        def acc_energy(type, energies, energy):
            if type in energies.keys():
                ...
                energies[type] = [energy_[i] + energy[i] for i in range(len(energy_))]
            else:
        ...
        def _opb_print(layer, stage_name):
            if OPB_PRINT and layer.off_traffic != 0:
                ...
                tflops = layer.get_flops() / exec_time / 1000 / 1000 / 1000 / 1000
                print(
                    "{},{},{},{},{},{}".format(
                        stage_name, batch_size, lin, layer.name, opb, tflops
                    )
                )

        def _pipeline(layers, level=False):
            qkv_time, prj_time, score_time, context_time, x2g_time, softmax_time = (
                0,
                0,
                0,
                0,
        ...
        def _ff_parallel(layers):
            bw_scale = (
                self.devices["Acc"].peak_memory_bandwidth
            ...
            for layer in layers:
                if "ff" in layer.name:
                    if layer.bound == "compute":
                        ...
                        attn_eff_bw = (
                            self.devices["GPU"].peak_memory_bandwidth * bw_scale / bs
                        )
                        ratio = self.devices["GPU"].peak_memory_bandwidth / (
                            self.devices["GPU"].peak_memory_bandwidth + attn_eff_bw
                        )
                        layer.exec_time *= ratio

        assert self.model_set, "Need to set_model"
        self.model.build(
            batch_size, lin, lout, self.hetero_name in [DeviceType.CPU, DeviceType.PIM]
        )
        ...
        for itr, bs in enumerate(target_bs):
            ...
            for gen_stage, decoder_block in enumerate(g_decoder):
                for l_idx, layer in enumerate(decoder_block):
                    if layer.type in [LayerType.MATMUL, LayerType.SOFTMAX, LayerType.X2G]:
                        exec_time, energy = self.devices["Acc"].get_time_and_energy(
                            layer, l_target=lin + lout
                        )
                    else:
                        exec_time, energy = self.devices["GPU"].get_time_and_energy(layer)
            ...
            energies = [
                ...
                unit_energy["g_reg"],
                unit_energy["g_alu"],
                gen_energies[LayerType.FC]["mem"],
                gen_energies[LayerType.FC]["comp"],
                gen_energies[LayerType.MATMUL]["mem"] + gen_energies[LayerType.SOFTMAX]["mem"],
                gen_energies[LayerType.MATMUL]["comp"] + gen_energies[LayerType.SOFTMAX]["comp"],
                gen_energies[LayerType.ACT]["mem"] + gen_energies[LayerType.NORM]["mem"],
                gen_energies[LayerType.ACT]["comp"] + gen_energies[LayerType.NORM]["comp"],
            ]
            ...
            perf = [t * self.model.ndec * 1000 for t in perf]  # ms
            energies = [t * self.model.ndec / 1000 for t in energies]  # nJ

            if itr == 0:
                if len(perf_all) > 0:
                    perf_all = [v + perf[i] * num_batches for i, v in enumerate(perf_all)]
                    energy_all = [
                        v + energy[i] * num_batches for i, v in enumerate(energy_all)
                    ]
                else:
        ...
        opb = self.devices["GPU"].peak_flops / self.devices["GPU"].peak_memory_bandwidth
        ...
        print(
            ...
            .format(
                batch_size,
                batch_size / ((perf_all[len(s_perf)]) / 1000),
                perf_all[len(s_perf)],
                pipe,
                parallel_ff,
                power_constraint,
    ...
    def get_required_mem_capacity(self, batch_size, lin, lout):
        ...
        w_byte = 2 if self.model.dtype in [DataType.W16A16, DataType.W16A8] else 1
        a_byte = 2 if self.model.dtype in [DataType.W16A16, DataType.W8A16] else 1
        l = lin + lout - 1
        ...
        if "LLAMA" in self.model.name:
            weight_memory = (
                ndec
                * hdim
                * (2 * hdim + 2 * (hdim) + 3 * ff_scale * hdim)
                * w_byte
            )
        else:
            weight_memory = (
                ndec * hdim * (2 * hdim + 2 * (hdim) + 2 * ff_scale * hdim) * w_byte
            )
